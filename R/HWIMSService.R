...
#' @param authentication HWIMS BASIC HTML authentication (list("user" = ..., "password" = ...))
#' @return dataframe with measurements
#' @export
x.hwims.request <- function(hwims.configuration, station, t.start, t.end, authentication){
  ...
  if(difftime(t.end, t.start, units=hwims.configuration[["interval.uom"]]) > hwims.configuration[["interval.max"]]) {
    df.time = x.time.intervals(t.start, t.end, hwims.configuration[["interval.max"]], hwims.configuration[["interval.uom"]])
  } else {
  ...
  for(i in 1:nrow(df.time)){
    ...
    request <- x.hwims.request(hwims.configuration, station, df.time[i, "t.start"], df.time[i, "t.end"])
    
    #request service
    response <- httr::POST(url=hwims.configuration[["url"]], body = request, httr::authenticate(user=authentication$user, password=authentication$password))
    ...
    if(nrow(df.measurements) == 0)
      warning(paste("no measurements returned for", station, t.start, t.end, sep=" : "))
